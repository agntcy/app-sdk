...
class MCPClient:
  ...
  # Specific wrappers
  async def call_tool(self, name: str, arguments: Dict[str, Any], request_id: int = 1) -> dict:
    return await self.call_mcp_method("tools/call", {"name": name, "arguments": arguments}, request_id=request_id)
  ...
  async def list_resource_templates(self, request_id: int = 1) -> dict:
    return await self.call_mcp_method("resources/templates/list", request_id=request_id)

  async def read_resource(self, uri: str, request_id: int = 1) -> dict:
    return await self.call_mcp_method("resources/read", {"uri": uri}, request_id=request_id)

  async def list_prompts(self, request_id: int = 1) -> dict:
    return await self.call_mcp_method("prompts/list", request_id=request_id)

  async def get_prompt(self, name: str, arguments: Optional[Dict[str, Any]] = None, request_id: int = 1) -> dict:
    return await self.call_mcp_method("prompts/get", {"name": name, "arguments": arguments or {}}, request_id=request_id)
  ...
  async def complete(
    ...
    raise NotImplementedError("The 'complete' method is not supported in this MCPClient implementation.")
