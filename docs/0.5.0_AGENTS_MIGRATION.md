...
| Method                         | Description                                                                                                                               |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `session.add(target)`          | Start building — `target` is the server instance (e.g., `A2AStarletteApplication`, `MCPServer`, `FastMCP`) or an `A2ASlimRpcServerConfig` |
...
| `.with_host(host)`             | Set the host — **required** for JSONRPC HTTP serving (when no transport is set)                                                            |
| `.with_port(port)`             | Set the port — **required** for JSONRPC HTTP serving (when no transport is set)                                                            |
